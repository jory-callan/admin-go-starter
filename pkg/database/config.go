...
type Config struct {
	...
	ConnMaxLifetime time.Duration `mapstructure:"conn_max_lifetime" yaml:"conn_max_lifetime" json:"conn_max_lifetime"`    // 连接最大存活时间(秒)
	ConnMaxIdleTime time.Duration `mapstructure:"conn_max_idle_time" yaml:"conn_max_idle_time" json:"conn_max_idle_time"` // 连接最大空闲时间(秒)
	LogLevel        string        `mapstructure:"log_level" yaml:"log_level" json:"log_level"`                            // gorm 日志级别
	SlowThreshold   time.Duration // 慢查询阈值（毫秒）
...
func DefaultConfig() Config {
	return Config{
		...
		MaxOpenConns:    50,
		MaxIdleConns:    10,
		ConnMaxLifetime: 30 * time.Minute, // 30 分钟
		ConnMaxIdleTime: 5 * time.Minute,  // 5 分钟
		LogLevel:        "warn",
